...
const toneClasses: Record<Tone, { border: string; text: string; button: string; tint: string }> = {
  amber: { border: 'border-amber-300/20', text: 'text-amber-100', button: 'bg-amber-300', tint: 'bg-amber-300/[0.04]' },
  rose: { border: 'border-rose-300/20', text: 'text-rose-100', button: 'bg-rose-300', tint: 'bg-rose-300/[0.04]' },
  indigo: { border: 'border-indigo-300/20', text: 'text-indigo-100', button: 'bg-indigo-300', tint: 'bg-indigo-300/[0.04]' },
  violet: { border: 'border-violet-300/20', text: 'text-violet-100', button: 'bg-violet-300', tint: 'bg-violet-300/[0.04]' },
  slate: { border: 'border-slate-300/20', text: 'text-slate-100', button: 'bg-slate-300', tint: 'bg-slate-300/[0.04]' },
};
...
export function ImbaEnterpriseWorkspace({ view }: { view: ImbaEnterpriseView }) {
  ...
  return <div className="space-y-5"><section className={`rounded-[24px] border bg-[linear-gradient(120deg,rgba(255,255,255,.045),rgba(255,255,255,.015))] p-6 ${tone.border}`}><div className="flex flex-wrap items-start justify-between gap-5"><div className="max-w-3xl"><p className={`text-[9px] font-black uppercase tracking-[0.24em] ${tone.text}`}>{config.section}</p><h2 className="mt-1 text-2xl font-semibold tracking-tight text-white">{config.title}</h2><p className="mt-2 text-sm leading-6 text-[#a5b7b1]">{config.description}</p></div><div className="rounded-2xl border border-amber-300/15 bg-amber-300/[0.05] px-4 py-3"><p className="text-[9px] font-black uppercase text-amber-100">Illustrative workspace</p><p className="mt-1 text-[10px] text-[#9caaa6]">Workflow depth shown without claiming internal access.</p></div></div></section><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{config.metrics.map((metric) => <div key={metric.label} className="rounded-[18px] border border-white/[0.08] bg-[#142321] p-4"><p className="text-[9px] font-black uppercase tracking-[0.18em] text-[#718981]">{metric.label}</p><p className={`mt-3 font-mono text-2xl font-semibold ${tone.text}`}>{metric.value}</p><p className="mt-1 text-[10px] text-[#81978f]">{metric.note}</p></div>)}</div><div className="grid gap-5 xl:grid-cols-12"><section className="rounded-[22px] border border-white/[0.08] bg-[#111b1a]/90 xl:col-span-8"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.07] px-5 py-4"><div><p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#718981]">Operating register</p><h3 className="mt-1 text-base font-semibold text-white">{config.title} queue</h3></div><label className="flex items-center gap-2 rounded-xl border border-white/[0.09] bg-white/[0.025] px-3 py-2"><Search className="h-3.5 w-3.5 text-[#718981]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search records" className="w-40 bg-transparent text-[10px] text-white outline-none placeholder:text-[#617971]" /></label></div><div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left"><thead><tr className="border-b border-white/[0.07] text-[9px] font-black uppercase tracking-[0.15em] text-[#6f8981]"><th className="px-5 py-3">Record</th><th className="px-3 py-3">{config.valueLabel}</th><th className="px-3 py-3">{config.ownerLabel}</th><th className="px-3 py-3">{config.dueLabel}</th><th className="px-5 py-3">Status</th></tr></thead><tbody>{records.map((record) => <tr key={record.id} onClick={() => { setSelectedId(record.id); setAction(''); }} className={`cursor-pointer border-b border-white/[0.055] last:border-0 hover:bg-white/[0.025] ${selected.id === record.id ? tone.tint : ''}`}><td className="px-5 py-3.5"><p className="text-xs font-semibold text-white">{record.name}</p><p className="mt-1 text-[9px] text-[#718981]">{record.id} · {record.secondary}</p></td><td className={`px-3 py-3.5 font-mono text-xs font-semibold ${tone.text}`}>{record.value}</td><td className="px-3 py-3.5 text-[10px] text-[#afc0bb]">{record.owner}</td><td className="px-3 py-3.5 text-[10px] text-[#afc0bb]">{record.due}</td><td className="px-5 py-3.5"><span className="rounded-full border border-white/[0.08] bg-white/[0.035] px-2 py-1 text-[8px] font-black uppercase text-white">{record.status}</span></td></tr>)}</tbody></table></div></section><section className="rounded-[22px] border border-white/[0.08] bg-[#111b1a]/90 xl:col-span-4"><div className="border-b border-white/[0.07] px-5 py-4"><p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#718981]">Selected · {selected.id}</p><h3 className="mt-1 text-base font-semibold text-white">{selected.name}</h3></div><div className="space-y-4 p-5"><p className="text-xs leading-6 text-[#a9bbb5]">{selected.detail}</p><div className="grid grid-cols-2 gap-3"><div className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3"><p className="text-[8px] font-black uppercase text-[#617971]">Owner</p><p className="mt-1 text-[10px] font-semibold text-white">{selected.owner}</p></div><div className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3"><p className="text-[8px] font-black uppercase text-[#617971]">Status</p><p className={`mt-1 text-[10px] font-semibold ${tone.text}`}>{selected.status}</p></div></div><div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4"><div className="flex items-center gap-2"><Database className={`h-3.5 w-3.5 ${tone.text}`} /><p className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Control posture</p></div><div className="mt-3 space-y-2">{['Owner and next action are explicit', 'Financial or compliance consequence is linked', 'Evidence and changes retain an audit trail'].map((item) => <div key={item} className="flex items-center gap-2 text-[9px] text-[#a9bbb5]"><ShieldCheck className="h-3.5 w-3.5 text-[rgb(var(--sa))]" />{item}</div>)}</div></div><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setAction('Owner notified and action recorded.')} className={`flex items-center justify-center gap-2 rounded-xl px-3 py-3 text-[9px] font-black uppercase text-[rgb(var(--sa-ink))] ${tone.button}`}><Send className="h-3.5 w-3.5" /> Assign</button><button type="button" onClick={() => setAction('Controlled export prepared with watermark.')} className="flex items-center justify-center gap-2 rounded-xl border border-white/[0.09] px-3 py-3 text-[9px] font-black uppercase text-white"><Download className="h-3.5 w-3.5" /> Export</button></div>{action ? <p className="flex items-center gap-2 text-[9px] text-[rgb(var(--sa-soft))]"><Check className="h-3.5 w-3.5" />{action}</p> : null}</div></section></div></div>;
}
