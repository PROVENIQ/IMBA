...
function TransactionsIntegrated({ onNavigate }: { onNavigate: (view: ImbaOsView) => void }) {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-violet-300/15 bg-violet-300/[0.045] px-4 py-3"><div><p className="text-[11px] font-black uppercase tracking-wider text-violet-700 dark:text-violet-100">QuickBooks write-back gate</p><p className="mt-1 text-[11px] text-[rgb(var(--text-2))]">The form creates an IMBA-OS draft. Release to QuickBooks requires mapping validation and approval.</p></div><div className="flex gap-2"><button type="button" onClick={() => setJobId(queueSync({ system: 'qbo', action: 'create', recordType: 'Transaction draft', recordId: `DRAFT-${Date.now().toString().slice(-5)}`, summary: 'Validated bill or invoice draft from Finance workspace', requiresApproval: true }))} className="rounded-xl bg-violet-300 px-3 py-2 text-[11px] font-black uppercase text-[#171126]">Stage current draft</button><button type="button" onClick={() => onNavigate('integration-sync')} className="rounded-xl border border-[rgb(var(--line)/0.1)] px-3 py-2 text-[11px] font-black uppercase text-[rgb(var(--text))]">Open sync queue</button></div>{jobId ? <p className="w-full text-[11px] font-semibold text-[rgb(var(--sa-soft))]">{jobId} staged; no QuickBooks posting has occurred.</p> : null}</div><Transactions /></div>;
}
  ...
  return (
    <>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <ShellCard><Heading eyebrow="Control signals" title="Finance action queue" /><div className="space-y-3 p-5">{['Collect Great Lakes invoice before next mobilization', 'Submit federal reimbursement package by Jul 31', 'Resolve equipment invoice decision hold', 'Refresh three project estimates to complete'].map((item, index) => <div key={item} className="flex items-start gap-3"><span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[11px] font-black ${index < 2 ? 'bg-rose-300/10 text-rose-700 dark:text-rose-100' : 'bg-amber-300/10 text-amber-800 dark:text-amber-100'}`}>{index + 1}</span><p className="text-[11px] leading-5 text-[rgb(var(--text-2))]">{item}</p></div>)}</div></ShellCard>
      </div>
    </>
  );
}
