...
function Transactions() {
  ...
  const fieldClass = 'mt-1.5 w-full rounded-xl border border-white/[0.09] bg-[#14201e] px-3 py-2.5 text-xs text-white outline-none focus:border-[rgb(var(--sa)/0.35)]';
  return <div className="grid gap-5 xl:grid-cols-12"><ShellCard className="xl:col-span-8"><div className="flex items-center justify-between gap-3 border-b border-white/[0.07] px-5 py-4"><div><p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#718981]">Controlled entry</p><h2 className="mt-1 text-base font-semibold text-white">{mode === 'bill' ? 'Enter vendor bill / chapter obligation' : 'Create client invoice / grant receivable'}</h2></div><div className="rounded-xl border border-white/[0.08] p-1"><button type="button" onClick={() => { setMode('bill'); setSaved(false); }} className={`rounded-lg px-3 py-2 text-[9px] font-black ${mode === 'bill' ? 'bg-[rgb(var(--sa))] text-[rgb(var(--sa-ink))]' : 'text-[#82978f]'}`}>Bill</button><button type="button" onClick={() => { setMode('invoice'); setSaved(false); }} className={`rounded-lg px-3 py-2 text-[9px] font-black ${mode === 'invoice' ? 'bg-[rgb(var(--sa))] text-[rgb(var(--sa-ink))]' : 'text-[#82978f]'}`}>Invoice</button></div></div><form onSubmit={(event) => { event.preventDefault(); setSaved(true); }} className="grid gap-4 p-5 md:grid-cols-2"><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">{mode === 'bill' ? 'Vendor / chapter' : 'Customer / funder'}<input required className={fieldClass} placeholder={mode === 'bill' ? 'Select counterparty' : 'Select customer'} /></label><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Amount<input required value={amount} onChange={(event) => setAmount(event.target.value)} className={fieldClass} placeholder="$0.00" /></label><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Project / chapter / grant<select required className={fieldClass}><option value="">Select controlling record</option><option>Great Lakes Buildout</option><option>Federal Recreation Partnership</option><option>Front Range MTB Alliance</option></select></label><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Account + function<select required className={fieldClass}><option>Choose canonical account</option><option>5200 · Project subcontractors</option><option>2140 · Due to chapters</option><option>4500 · Trail Solutions revenue</option></select></label><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Transaction date<input type="date" className={fieldClass} /></label><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Due / milestone date<input type="date" className={fieldClass} /></label><label className="text-[9px] font-black uppercase tracking-wider text-[#718981] md:col-span-2">Memo + evidence<textarea className={`${fieldClass} min-h-24`} placeholder="Business purpose, approval context, deliverable, or restriction..." /></label><div className="flex items-center justify-between gap-3 md:col-span-2"><p className="text-[9px] text-[#718981]">Required dimensions are validated before submission reaches approval.</p><button type="submit" className="rounded-xl bg-[rgb(var(--sa))] px-5 py-3 text-[10px] font-black uppercase text-[rgb(var(--sa-ink))]">Submit for approval</button></div>{saved ? <p className="flex items-center gap-2 text-[10px] text-[rgb(var(--sa-soft))] md:col-span-2"><Check className="h-3.5 w-3.5" /> Demo {mode} for {amount || '$0'} entered into the approval queue.</p> : null}</form></ShellCard><ShellCard className="xl:col-span-4"><Heading eyebrow="Transaction controls" title="What must travel with the money" /><div className="space-y-3 p-5">{['Counterparty identity and approval', 'Canonical account and functional class', 'Project, phase, chapter, or grant tag', 'Restriction / allowability evidence', 'Billing or payment milestone', 'Source document and audit trail'].map((item, index) => <div key={item} className="flex items-center gap-3 rounded-xl border border-white/[0.06] px-3 py-2.5"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-[rgb(var(--sa)/0.10)] font-mono text-[8px] text-[rgb(var(--sa-soft))]">{index + 1}</span><span className="text-[10px] text-[#afc0bb]">{item}</span></div>)}</div></ShellCard></div>;
}
        ...
        <ShellCard className="xl:col-span-8">
          ...
          <div className="grid gap-3 p-5 md:grid-cols-2">
            {launchers.map((item) => {
              ...
              return (
                <button key={item.title} type="button" onClick={() => onNavigate(item.view)} className="group rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4 text-left transition hover:border-[rgb(var(--sa)/0.25)] hover:bg-[rgb(var(--sa)/0.04)]">
                  <div className="flex items-start justify-between gap-3"><span className="rounded-xl bg-[rgb(var(--sa)/0.10)] p-2 text-[rgb(var(--sa-soft))]"><Icon className="h-4 w-4" /></span><ArrowRight className="h-4 w-4 text-[#607870] transition group-hover:translate-x-1 group-hover:text-[rgb(var(--sa))]" /></div>
                  <h3 className="mt-4 text-sm font-semibold text-white">{item.title}</h3>
                  <div className="mt-2 flex items-center justify-between gap-3 text-[10px]"><span className="text-[#7f958e]">{item.note}</span><span className="font-mono font-semibold text-[rgb(var(--sa-soft))]">{item.value}</span></div>
                </button>
              );
            })}
          </div>
          ...
          <div className="space-y-2 p-5">
            {[
              ...
              <div key={label} className="flex items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2.5">
                ...
                <span className={`rounded-full px-2 py-1 text-[8px] font-black uppercase ${status === 'Complete' ? 'bg-[rgb(var(--sa)/0.10)] text-[rgb(var(--sa-soft))]' : status === 'Review' ? 'bg-cyan-300/10 text-cyan-100' : 'bg-amber-300/10 text-amber-100'}`}>{status}</span>
              </div>
            ))}
          </div>
        </ShellCard>
...
function GrantsIntegrated({ onNavigate }: { onNavigate: (view: ImbaOsView) => void }) {
  ...
  return <div className="space-y-4"><div className="grid gap-4 rounded-2xl border border-[rgb(var(--sa)/0.15)] bg-[rgb(var(--sa)/0.04)] p-4 lg:grid-cols-[1.2fr_1fr_1fr_auto]"><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Award<select value={selectedId} onChange={(event) => setSelectedId(event.target.value)} className="mt-1.5 w-full rounded-xl border border-white/[0.09] bg-[#14201e] px-3 py-2.5 text-xs text-white outline-none">{imbaGrants.map((grant) => <option key={grant.id} value={grant.id}>{grant.id} · {grant.funder}</option>)}</select></label><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Allowable spend<input type="number" min="0" max={selected.award} value={selected.spent} onChange={(event) => updateSpent(Number(event.target.value))} className="mt-1.5 w-full rounded-xl border border-white/[0.09] bg-[#14201e] px-3 py-2.5 text-xs text-white outline-none" /></label><label className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Control status<select value={selected.status} onChange={(event) => updateRecord('grant', selected.id, { status: event.target.value }, { actor: 'Grant accountant', detail: `Updated grant control status for ${selected.id}.` })} className="mt-1.5 w-full rounded-xl border border-white/[0.09] bg-[#14201e] px-3 py-2.5 text-xs text-white outline-none"><option>On track</option><option>Draw due</option><option>Watch</option></select></label><div className="self-end rounded-xl border border-white/[0.08] px-4 py-2.5"><p className="text-[8px] font-black uppercase text-[rgb(var(--sa-soft))]">Control subledger</p><p className="mt-1 text-[9px] text-[#91a49e]">Edits persist · QBO tie-out queued · audit retained</p></div></div><Grants onNavigate={onNavigate} /></div>;
}
...
function Grants({ onNavigate }: { onNavigate: (view: ImbaOsView) => void }) {
  ...
  return (
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        <ShellCard className="xl:col-span-8">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.07] px-5 py-4"><div><p className="text-[9px] font-black uppercase tracking-[0.22em] text-[#718981]">Award register</p><h2 className="mt-1 text-base font-semibold text-white">Restriction + draw + deadline control</h2></div><div className="flex gap-1">{['All', 'On track', 'Draw due', 'Watch'].map((item) => <button key={item} type="button" onClick={() => setStatus(item)} className={`rounded-lg px-2.5 py-2 text-[9px] font-bold ${status === item ? 'bg-[rgb(var(--sa))] text-[rgb(var(--sa-ink))]' : 'border border-white/[0.08] text-[#94a8a1]'}`}>{item}</button>)}</div></div>
          <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left"><thead><tr className="border-b border-white/[0.07] text-[9px] font-black uppercase tracking-[0.16em] text-[#6f8981]"><th className="px-5 py-3">Funder + program</th><th className="px-3 py-3 text-right">Award</th><th className="px-3 py-3">Burn</th><th className="px-3 py-3 text-right">Remaining</th><th className="px-3 py-3">Next deadline</th><th className="px-5 py-3">Status</th></tr></thead><tbody>{rows.map((grant) => { const burn = Math.round((grant.spent / grant.award) * 100); return <tr key={grant.id} onClick={() => setSelectedId(grant.id)} className={`cursor-pointer border-b border-white/[0.055] last:border-0 hover:bg-white/[0.03] ${selectedId === grant.id ? 'bg-[rgb(var(--sa)/0.035)]' : ''}`}><td className="px-5 py-3.5"><p className="text-xs font-semibold text-white">{grant.funder}</p><p className="mt-1 text-[9px] text-[#718981]">{grant.id} · {grant.program}</p></td><td className="px-3 py-3.5 text-right font-mono text-xs text-[#b5c5c0]">{money(grant.award)}</td><td className="px-3 py-3.5"><div className="flex items-center gap-2"><div className="h-1.5 w-20 overflow-hidden rounded-full bg-white/[0.08]"><div className="h-full rounded-full bg-[#68b9aa]" style={{ width: `${burn}%` }} /></div><span className="font-mono text-[9px] text-[#9fb2ac]">{burn}%</span></div></td><td className="px-3 py-3.5 text-right font-mono text-xs text-[rgb(var(--sa-soft))]">{money(grant.remaining)}</td><td className="px-3 py-3.5 text-[10px] text-[#afc0bb]">{grant.nextDeadline}</td><td className="px-5 py-3.5"><span className={`rounded-full px-2 py-1 text-[8px] font-black uppercase ${grant.status === 'On track' ? 'bg-[rgb(var(--sa)/0.10)] text-[rgb(var(--sa-soft))]' : grant.status === 'Draw due' ? 'bg-cyan-300/10 text-cyan-100' : 'bg-amber-300/10 text-amber-100'}`}>{grant.status}</span></td></tr>; })}</tbody></table></div>
        </ShellCard>
        <ShellCard className="xl:col-span-4"><Heading eyebrow={`${selected.id} · selected award`} title={selected.funder} /><div className="space-y-4 p-5"><div><p className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Restriction</p><p className="mt-1 text-xs leading-5 text-white">{selected.restriction}</p></div><div className="grid grid-cols-2 gap-3"><Kpi label="Remaining" value={money(selected.remaining)} note="Award balance" /><Kpi label="Draw pending" value={money(selected.reimbursement)} note="Receivable / action" tone={selected.reimbursement ? 'amber' : 'teal'} /></div><div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4"><p className="text-[9px] font-black uppercase tracking-wider text-[#718981]">Next controls</p><div className="mt-3 space-y-2">{['Validate allowable-cost coding', 'Reconcile payroll allocation', 'Attach evidence to reimbursement', 'Route narrative data to program owner'].map((item, index) => <div key={item} className="flex items-center gap-2 text-[10px] text-[#a9bbb5]">{index < 2 ? <Check className="h-3.5 w-3.5 text-[rgb(var(--sa))]" /> : <CalendarClock className="h-3.5 w-3.5 text-amber-200" />}{item}</div>)}</div></div><button type="button" onClick={() => onNavigate('development-grant-pipeline')} className="w-full rounded-xl bg-[rgb(var(--sa))] px-4 py-3 text-[10px] font-black uppercase tracking-wider text-[rgb(var(--sa-ink))]">Open grant workspace</button></div></ShellCard>
      </div>
    </>
  );
}
    ...
    <>
      ...
      <ShellCard>
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.07] px-5 py-4">
          ...
          <p className="text-[9px] text-[#718981]">Vendor bills live in <span className="text-[rgb(var(--sa-soft))]">Money → Accounts payable</span></p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-left">
            ...
            <tbody>{receivables.map((row) => (
              <tr key={row.ref} className={`border-b border-white/[0.055] last:border-0 ${selectedReceivable?.ref === row.ref ? 'bg-[rgb(var(--sa)/0.035)]' : ''}`}>
        ...
        {selectedReceivable ? (
          <div className="grid gap-4 border-t border-[rgb(var(--sa)/0.15)] bg-[rgb(var(--sa)/0.025)] p-5 lg:grid-cols-[1.4fr_1fr_auto]">
            ...
            <div className="flex items-start gap-2"><button type="button" disabled={selectedReceivable.followUpQueued} onClick={() => queueFollowUp(selectedReceivable)} className="rounded-xl bg-[rgb(var(--sa))] px-4 py-3 text-[9px] font-black uppercase text-[rgb(var(--sa-ink))] disabled:bg-[rgb(var(--sa)/0.10)] disabled:text-[rgb(var(--sa-soft))]">{selectedReceivable.followUpQueued ? 'Follow-up queued' : 'Queue follow-up'}</button><button type="button" aria-label="Close receivable detail" onClick={() => setSelectedRef(null)} className="rounded-xl border border-white/[0.1] p-3 text-[#94aaa3]"><X className="h-4 w-4" /></button></div>
          </div>
        ) : null}
      </ShellCard>
    </>
