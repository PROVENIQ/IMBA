    ...
    <section className="rounded-[20px] border border-violet-300/15 bg-[linear-gradient(110deg,rgba(167,139,250,.075),rgba(104,185,170,.045))] p-3.5">
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <button type="button" onClick={onOpenAlerts} className="relative inline-flex items-center gap-2 rounded-xl bg-[#b7e35b] px-3 py-2.5 text-[8px] font-black uppercase tracking-wider text-[#102016] hover:bg-[#c9ef79]"><Bell className="h-3.5 w-3.5" /> Alerts + briefings{activeAlertCount ? <span className="rounded-full bg-[#102016]/15 px-1.5 py-0.5">{activeAlertCount}</span> : null}</button>
        </div>
      </div>
...
export function ImbaInsightStrip({ insights, onSelect }: { insights: ImbaInsight[]; onSelect: (insight: ImbaInsight) => void }) {
  const tone = { positive: 'border-[#b7e35b]/15 bg-[#b7e35b]/[0.045] text-[#dff7a8]', warning: 'border-amber-300/15 bg-amber-300/[0.045] text-amber-100', risk: 'border-rose-300/15 bg-rose-300/[0.045] text-rose-100' };
  return (
    ...
    <div className="fixed inset-0 z-[180]" role="dialog" aria-modal="true" aria-label={`Explain ${selection.label}`}>
      ...
      <aside className="absolute inset-y-0 right-0 flex w-full max-w-[520px] flex-col border-l border-white/[0.1] bg-[#091411] shadow-[-24px_0_80px_rgba(0,0,0,.42)]">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-5">
          ...
          <section><p className="flex items-center gap-2 text-[9px] font-black uppercase tracking-wider text-[#dff7a8]"><CircleDollarSign className="h-3.5 w-3.5" /> Principal drivers</p><div className="mt-2 space-y-2">{explanation.drivers.map((driver) => <div key={driver.label} className="flex items-center justify-between gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2.5"><span className="text-[10px] text-[#b5c5c0]">{driver.label}</span><span className={`text-[9px] font-black ${driver.tone === 'positive' ? 'text-[#dff7a8]' : driver.tone === 'negative' ? 'text-rose-100' : 'text-[#9fd6cc]'}`}>{driver.effect}</span></div>)}</div></section>
          ...
          <section className="rounded-2xl border border-[#b7e35b]/15 bg-[#b7e35b]/[0.045] p-4"><p className="text-[9px] font-black uppercase tracking-wider text-[#dff7a8]">Management action</p><p className="mt-2 text-[10px] leading-5 text-[#c8d2ce]">{explanation.action}</p></section>
        </div>
        <div className="border-t border-white/[0.07] p-5"><button type="button" onClick={() => { onNavigate(explanation.supportingView); onClose(); }} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#b7e35b] px-4 py-3 text-[9px] font-black uppercase tracking-wider text-[#102016]">Open supporting records <ArrowRight className="h-3.5 w-3.5" /></button></div>
      </aside>
    </div>
  );
}
