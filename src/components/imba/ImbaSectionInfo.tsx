...
export function ImbaSectionInfo({
  ...
  return (
    <span className="group/si relative inline-flex shrink-0 items-center">
      ...
      <span
        role="tooltip"
        className="pointer-events-none absolute left-0 top-[calc(100%+10px)] z-[130] hidden w-80 rounded-2xl border border-[rgb(var(--line)/0.12)] bg-[rgb(var(--panel)/95%)] p-4 text-left shadow-[0_20px_50px_rgba(0,0,0,0.55)] backdrop-blur-md group-hover/si:block group-focus-within/si:block"
      >
        <span className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-[rgb(var(--sa))]" />
          <span className="text-[11px] font-bold text-[rgb(var(--text))]">{section}</span>
        </span>
        <span className="mt-3 block text-[11px] font-black uppercase tracking-[0.16em] text-[rgb(var(--text-3))]">
          Where the data comes from
        </span>
        <ul className="mt-1.5 space-y-1.5">
          {sources.map((source, index) => (
            <li key={index} className="flex gap-2 text-[11px] leading-4 text-[rgb(var(--text))]">
        ...
        {note ? (
          <span className="mt-3 block border-t border-[rgb(var(--line)/0.08)] pt-2.5 text-[11px] leading-4 text-[rgb(var(--text-2))]">
            {note}
          </span>
        ) : null}
      </span>
